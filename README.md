...
* [`src/cljs/re_frame_swapi/`](src/cljs/re_frame_swapi/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/re_frame_swapi/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `re-frame-swapi` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/re_frame_swapi/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns re-frame-swapi.example
  (:require [re-frame-swapi.config :as config])
